...
/**
 * Interface for XML event listeners. <h3>State transitions</h3> XML event sources must obey a state
 * model when calling <code>XMLEventListener</code>s. However, an <code>XMLEventListener</code> is
 * not required to check that this state model is actually respected. If it
 * does, then it will throw an {@link IllegalStateException} if the state
 * model is violated.
 * <p>
 * Stateful <code>XMLEventListener</code> implementations should implement the
 * {@link StatefulXMLEventListener} interface instead of implementing <code>XMLEventListener</code>
 * directly.
 * <p>
 * Initially the state of an uninitialized <code>XMLEventListener</code> is {@link #UNINITIALIZED
 * UNINITIALIZED}.
 * <p>
 * The following table defines how the state changes when a certain method is called in a certain
 * state. Horizontally are the current states, vertically the notification methods. The cells self
 * contain the new state.
 * <p>
 * <table class="states">
 * <tr>
 * <th></th>
 ...
 * <td class="err"><acronym title="IllegalStateException">ISE</acronym></td>
 * <td class="err"><acronym title="IllegalStateException">ISE</acronym></td>
 * <td><acronym title="WITHIN_ELEMENT">S4</acronym>/<acronym title="AFTER_ROOT_ELEMENT">S5</acronym>
 * </td>
 * <td><acronym title="WITHIN_ELEMENT">S4</acronym>/<acronym title="AFTER_ROOT_ELEMENT">S5</acronym>
 * </td>
 * <td class="err"><acronym title="IllegalStateException">ISE</acronym></td>
...
public interface XMLEventListener extends XMLEventListenerStates {
    ...
    /**
     * Resets this XML event listener. The state will be set to {@link #UNINITIALIZED UNINITIALIZED}
     * .
     */
    void reset();
    ...
    /**
     * Notification of a comment. The comment should not contain the string <code>"--"</code>.
     * <p>
     * If the state equals {@link #BEFORE_XML_DECLARATION}, then it will be set to
     * {@link #BEFORE_DTD_DECLARATION}, otherwise if the state is {@link #START_TAG_OPEN} then it
     * will be set to {@link #WITHIN_ELEMENT}, otherwise the state will not be changed.
